"""
TCN Model Trainer for Sequential Recommendation
"""
...
class TCNTrainer:
    ...
    def build_model(self, vocab_size: int, max_seq_length: int) -> keras.Model:
        ...
        # TCN layers
        if TCN_AVAILABLE:
            x = self._build_tcn_layer(embedding)
        else:
            x = self._build_manual_tcn(embedding)
        ...
        outputs = layers.Dense(
            vocab_size,
            activation='softmax',
            dtype='float32',  # Use float32 for output layer
            name='predictions'
        )(x)
        
        model = keras.Model(inputs=inputs, outputs=outputs, name='tcn_recommender')
        
        return model
    
    def _build_tcn_layer(self, embedding):
        """Build TCN layer using keras-tcn library"""
        
        x = TCN(
            nb_filters=self.config.nb_filters,
            kernel_size=self.config.kernel_size,
            nb_stacks=self.config.nb_stacks,
    ...
    def _build_manual_tcn(self, embedding):
        """Manual TCN implementation using Conv1D layers"""
        
        x = embedding
        
        # Stack of dilated convolutions
        for stack in range(self.config.nb_stacks):
            for i, dilation_rate in enumerate(self.config.dilations):
                # Dilated convolution
                conv = layers.Conv1D(
                    ...
                    padding='causal',
                    activation=None,
                    name=f'conv1d_stack{stack}_dilation{i}'
                )(x)
                
                # Batch normalization
                conv = layers.BatchNormalization(name=f'bn_stack{stack}_dilation{i}')(conv)
                
                # Activation
                conv = layers.Activation(self.config.activation, name=f'act_stack{stack}_dilation{i}')(conv)
                
                # Dropout
                conv = layers.Dropout(self.config.dropout_rate, name=f'dropout_stack{stack}_dilation{i}')(conv)
                
                # Residual connection if dimensions match
                if self.config.use_skip_connections:
                    if x.shape[-1] == conv.shape[-1]:
                        x = layers.Add(name=f'residual_stack{stack}_dilation{i}')([x, conv])
                    else:
                        # Project x to match conv dimensions
                        x_proj = layers.Conv1D(
                            filters=self.config.nb_filters,
                            kernel_size=1,
                            padding='same',
                            name=f'projection_stack{stack}_dilation{i}'
                        )(x)
                        x = layers.Add(name=f'residual_stack{stack}_dilation{i}')([x_proj, conv])
                else:
                    x = conv
    ...
    def train(self,
        ...
        logger.info(f"TCN model built with vocab_size={vocab_size}, seq_length={max_seq_length}")
        ...
        try:
            ...
            logger.info(f"TCN training completed - Val Loss: {self.training_metrics['val_loss']:.4f}")
            
        except Exception as e:
            logger.error(f"Error during TCN training: {str(e)}")
            # Fallback training without eager execution
            ...
            self.history = self.model.fit(
                ...
                targets,
                batch_size=self.config.batch_size,
                epochs=min(5, self.config.epochs),  # Reduced epochs for fallback
                validation_split=validation_split,
                verbose=1
            )
            
            self.training_metrics = self._calculate_training_metrics()
            logger.info(f"TCN fallback training completed - Val Loss: {self.training_metrics['val_loss']:.4f}")
    ...
    def save_model(self, model_path: Path, timestamp: Optional[str] = None) -> Path:
        ...
        try:
            # Save model
            model_file = model_path / f"tcn_model_{timestamp}.h5"
            self.model.save(model_file)
            logger.info(f"TCN model saved to {model_file}")
            
        except Exception as e:
            logger.warning(f"Failed to save TCN model normally: {str(e)}")
            # Try to save weights only
            weights_file = model_path / f"tcn_weights_{timestamp}.h5"
            self.model.save_weights(weights_file)
            
            # Save model config
            config_file = model_path / f"tcn_config_{timestamp}.json"
            model_config = self.model.get_config()
            save_json(model_config, config_file)
            
            model_file = weights_file
            logger.info(f"TCN model weights and config saved to {model_path}")
        
        # Save training history
        if self.history is not None:
            history_file = model_path / f"tcn_training_history_{timestamp}.csv"
            history_df = pd.DataFrame(self.history.history)
            history_df.to_csv(history_file, index=False)
        
        # Save training results
        results_file = model_path / f"tcn_training_results_{timestamp}.json"
    ...
    def load_model(self, model_path: Path):
        ...
        try:
            ...
            logger.info(f"TCN model loaded from {model_path}")
        except Exception as e:
